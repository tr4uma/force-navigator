 ...
 */
    ...
    function initShortcuts() {
      ...
      listenFor('Get Settings', function(response) {
    ...
    function sendMessage(name, data, responseFn) {
        if(chrome != null) chrome.extension.sendMessage({"name": name, {"data":data}}, responseFn);
        else if (safari != null) safari.self.tab.dispatchMessage(name,data);
    }

    function listenFor(name, responseFn) {
        if(listeners[name] != null) {
            // if(chrome != null) chrome.extension.removeListener(listeners[name]);
            // else if(safari != null) safari.self.removeEventListener(listeners[name]);
        }
        listeners[name] = responseFn;

        // if(chrome != null) chrome.extension.onMessage.addListener(listeners[name]);
        // else if(safari != null) safari.self.tab.addEventListener("message",listeners[name]);
    }

    function addListeners() {

        if(chrome != null) chrome.extension.onMessage.addListener(messageListener);
        else if(safari != null) safari.self.tab.addEventListener("message",messageListener);
    }

    function messageListener(msg) {
        if(listeners[msg.name] != null) listeners[msg.name].apply(null, msg.message.data);
    }
    ...
    function init()
    {
        ...
        sendMessage('Get Commands', {'key': hash});
        listenFor('Get Commands', ,
          function(response) {
            cmds = response;
        ...
        sendMessage('Get Metadata', {'key': hash});
        listenFor('Get Metadata', function(response) {
            metaData = response;
        });
